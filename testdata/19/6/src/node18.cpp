  ...
  std::mt19937 gen(rd());
  std::normal_distribution<> d(0.020406, 0.005);
  class Node18{
  	public:
  		Node18(){
			sub_11_18_flag = 0;
  			sub_11_18 = n.subscribe("topic_11_18", 1, &Node18::middlemanCallback11,this);
			sub_17_18_flag = 0;
		...
		void middlemanCallback11(const std_msgs::String::ConstPtr& msg){
  			if(sub_17_18_flag == 1 &&  true){
  				...
  				ROS_INFO("I'm node18 last from node11, intercepted: [%s]", msg->data.c_str());
				pub_18_19.publish(msg);
				sub_17_18_flag = 0;
			}
  			else{
  				ROS_INFO("I'm node18, from node11 intercepted: [%s]", msg->data.c_str());
  				sub_11_18_flag = 1;
  			}
  		}
		void middlemanCallback17(const std_msgs::String::ConstPtr& msg){
  			if(sub_11_18_flag == 1 &&  true){
  				usleep(d(gen)*1000000);
